...
export default function FacebookLogin({ authenticated }: FacebookLoginProps) {
  ...
  const [fullCredentials, setFullCredentials] = useState<Record<string, unknown> | null>(null);
  ...
  const loadFullCredentials = async () => {
    try {
      const response = await fetch('http://localhost:8000/facebook/credentials', {
        credentials: 'include'
      });
      if (response.ok) {
  ...
  const loadFacebookDataFromDatabase = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Facebook status response not ok, status:', response.status);
        setConnectionStatus('disconnected');
      }
    } catch (error) {
  ...
  const handleDisconnect = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/facebook/disconnect', {
        method: 'POST',
        credentials: 'include',
      });
      ...
      <div className="px-6 py-6">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Facebook Business</h3>
              <p className="text-sm text-gray-500">Connect your Facebook Business account</p>
            </div>
          </div>
        ...
        {!authenticated && (
          <div className="text-center py-8">
            <div className="text-gray-500 mb-4">
              ...
              <p className="text-sm">Please log in to connect your Facebook Business account.</p>
            </div>
          </div>
        )}
        ...
        {authenticated && isConnected && facebookData ? (
          <div className="space-y-6">
            <div className="bg-green-50 border border-green-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  ...
                  <span className="text-green-800 font-medium">Successfully connected to Facebook!</span>
                </div>
                <button
                  onClick={() => setShowCredentials(!showCredentials)}
                  className="text-sm text-blue-600 hover:text-blue-800 font-medium"
                >
                  {showCredentials ? 'Hide Credentials' : 'Show All Credentials'}
                </button>
              </div>
              
              <div className="text-sm text-green-700">
                <p><strong>User:</strong> {facebookData.facebook_user.name}</p>
                {facebookData.facebook_user.email && (
                  <p><strong>Email:</strong> {facebookData.facebook_user.email}</p>
                )}
              </div>
            </div>
            ...
            {showCredentials && fullCredentials && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <h4 className="text-md font-medium text-gray-900 mb-4">Facebook API Credentials</h4>
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Facebook User ID</label>
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <button
                        onClick={() => copyToClipboard(String(fullCredentials.facebook_user_id), 'Facebook User ID')}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Main Access Token</label>
                    <div className="flex items-center space-x-2">
                      <input
                        type="text"
                      ...
                      />
                      <button
                        onClick={() => copyToClipboard(String(fullCredentials.access_token), 'Access Token')}
                        className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                      >
                        Copy
                      </button>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <button
                        onClick={() => copyToClipboard(String(fullCredentials.facebook_user_name), 'Full Name')}
                        className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                      >
                        Copy
                      </button>
                  ...
                  {fullCredentials.facebook_user_email && (
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        />
                        <button
                          onClick={() => copyToClipboard(String(fullCredentials.facebook_user_email), 'Email')}
                          className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                        >
                          Copy
                        </button>
            ...
            {facebookData.pages.length > 0 && (
              <div>
                <h4 className="text-md font-medium text-gray-900 mb-3">Connected Facebook Pages ({facebookData.pages.length})</h4>
                        ...
                        <div className="space-y-3 pt-3 border-t border-gray-100">
                          <div>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              />
                              <button
                                onClick={() => copyToClipboard(page.id, `Page ID for ${page.name}`)}
                                className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                              >
                                Copy
                              </button>
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              />
                              <button
                                onClick={() => copyToClipboard(page.access_token, `Access Token for ${page.name}`)}
                                className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                              >
                                Copy
                              </button>
            ...
            {facebookData.instagram_accounts.length > 0 && (
              <div>
                <h4 className="text-md font-medium text-gray-900 mb-3">Connected Instagram Business Accounts ({facebookData.instagram_accounts.length})</h4>
                <div className="space-y-3">
                    ...
                    <div key={account.id} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <div className="text-xs text-gray-400 text-right">
                          {account.followers_count && <p>{account.followers_count.toLocaleString()} followers</p>}
                          {account.media_count && <p>{account.media_count} posts</p>}
                        </div>
                      </div>
                        ...
                        <div className="space-y-3 pt-3 border-t border-gray-100">
                          <div>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              />
                              <button
                                onClick={() => copyToClipboard(account.id, `Instagram ID for @${account.username}`)}
                                className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                              >
                                Copy
                              </button>
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              />
                              <button
                                onClick={() => copyToClipboard(account.username, 'Instagram Username')}
                                className="px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm"
                              >
                                Copy
                              </button>
          ...
          <div className="text-center py-8">
            ...
            <h4 className="text-lg font-medium text-gray-900 mb-2">Connect Facebook Business</h4>
            <p className="text-gray-500 mb-6">
              Connect your Facebook Business account to manage pages and Instagram accounts
            </p>
